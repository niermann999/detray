...
struct matrix_helper {
    ...
    inline matrix_type<3, 3> column_wise_cross(const matrix_type<3, 3>& m,
        ...
        auto m_col0 = getter::vector<3>(m, 0, 0);
        auto m_col1 = getter::vector<3>(m, 0, 1);
        auto m_col2 = getter::vector<3>(m, 0, 2);

        getter::set_block(ret, vector::cross(m_col0, v), 0, 0);
        getter::set_block(ret, vector::cross(m_col1, v), 0, 1);
        getter::set_block(ret, vector::cross(m_col2, v), 0, 2);
    ...
    DETRAY_HOST_DEVICE inline matrix_type<N, N> cholesky_decompose(
        ...
        auto L = matrix::zero<matrix_type<N, N>>();
