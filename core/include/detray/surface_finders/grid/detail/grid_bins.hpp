...
///
/// Keeps an additional counter to track the number of entries per bin. Bin
/// capaciy is static.
...
class static_array
    ...
    public:
    using entry_type = entry_t;
    using bin_view_type = detray::ranges::subrange<std::array<entry_t, N>>;
    using const_bin_view_type =
        detray::ranges::subrange<const std::array<entry_t, N>>;

    using iterator_t = detray::ranges::iterator_t<bin_view_type>;
    ...
    /// @returns view iterator over bin content in start position
    /// @{
    DETRAY_HOST_DEVICE
    auto begin() -> typename bin_view_type::iterator_t {
        return detray::ranges::begin(view());
    }
    DETRAY_HOST_DEVICE
    ...
    DETRAY_HOST_DEVICE auto view() const -> const_bin_view_type {
        return const_bin_view_type(m_content, dindex_range{0u, m_size});
    }

    /// @returns the subrange on the valid bin content
    DETRAY_HOST_DEVICE auto view() -> bin_view_type {
        return bin_view_type(m_content, dindex_range{0u, m_size});
    }
...
/// @brief Bin that views a collection of entries they it does not own.
///
...
class dynamic_array
    ...
    using bin_view_type = detray::ranges::subrange<container_t>;
    using const_bin_view_type = detray::ranges::subrange<const container_t>;
    ...
    /// @returns the subrange on the valid bin content - const
    DETRAY_HOST_DEVICE auto view() const -> const_bin_view_type {
        return const_bin_view_type(*m_global_storage,
                                   dindex_range{m_offset, m_offset + m_size});
    }

    /// @returns the subrange on the valid bin content
    DETRAY_HOST_DEVICE auto view() -> bin_view_type {
        return bin_view_type(*m_global_storage,
                             dindex_range{m_offset, m_offset + m_size});
    }
