...
struct bound_track_vector {
    ...
    /// Default constructor
    bound_track_vector() = default;

    /// Construct from a 6-dim vector of parameters
    DETRAY_HOST_DEVICE
    explicit bound_track_vector(const vector_type& vec) : m_vector(vec) {}
    ...
    /// @param t the time
    DETRAY_HOST_DEVICE
    bound_track_vector(const point2_type& loc_p, const scalar_type phi,
...
struct bound_track_parameters : public bound_track_vector<algebra_t> {
    ...
    DETRAY_HOST_DEVICE
    void set_vector(const track_vector_type& v) { this->m_vector = v.m_vector; }
