...
struct pointwise_material_interactor : actor {
    ...
    using bound_track_param_type = bound_track_parameters<algebra_t>;
    ...
    DETRAY_HOST_DEVICE inline void update(
        ...
        // ignored.
        const scalar_type approach{bound_params.bound_local()[e_bound_loc0]};
    ...
    DETRAY_HOST_DEVICE
    inline void update_qop(
        typename bound_track_param_type::track_vector_type &bound_vec,
        const pdg_particle<scalar_type> &ptc, const scalar_type e_loss,
        const int sign) const {
        const scalar_type m = ptc.mass();
        const scalar_type q = ptc.charge();
        const scalar_type p = bound_vec.p(q);
        ...
        bound_vec.set_qop((nextP == 0.f) ? inv
                          : (q != 0.f)   ? q / nextP
                                         : 1.f / nextP);
    }
