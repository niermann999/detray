...
class barcode {
    ...
    /// Construct default barcode with all values set to one.
    ...
    constexpr bool is_invalid() const {
        return ((volume() == (1UL << 12) - 1UL) |
                (static_cast<value_t>(id()) == (1UL << 4) - 1UL) |
                (index() == (1UL << 40) - 1UL));
    }
...
// compatibility.
    ...
    static constexpr int extractShift(value_t mask) {
        if (mask == k_volume_mask) {
            return 52;
        } else if (mask == k_id_mask) {
            return 48;
        } else if (mask == k_index_mask) {
            return 8;
        } else if (mask == k_extra_mask) {
            return 0;
        } else {
            assert(false && "invalid mask");
            return -1;
        }
    ...
    /// Extract the bit shift necessary to access the masked values - host/SYCL
    ///
    /// @note undefined behaviour for mask == 0 which we should not have.
    DETRAY_HOST
    static constexpr int extractShift(value_t mask) {
