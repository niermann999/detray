...
class surface {
    ...
    DETRAY_HOST_DEVICE
    constexpr auto global_to_local(const context &ctx, const point3 &global,
                                   const vector3 &dir) const {
        return visit_mask<typename kernels::global_to_local>(transform(ctx),
                                                             global, dir);
    }

    /// @returns the global position to the given 3D local position @param local
    /// for a given geometry context @param ctx
    DETRAY_HOST_DEVICE constexpr point3 local_to_global(
        const context &ctx, const point3 &local) const {
