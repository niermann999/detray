...
class tracking_surface {
    ...
    /// Track parameters in local (bound) coordinates
    using bound_param_type = typename kernels::bound_param_type;
    ...
    /// @returns the track parametrization and its covariance matrix @param cov
    /// projected onto the surface (bound)
    DETRAY_HOST_DEVICE
    constexpr auto free_to_bound_param(
        const context &ctx, const free_param_type &free_param,
        const typename bound_param_type::covariance_type &cov = {}) const {
        return visit_mask<typename kernels::free_to_bound_param>(
            transform(ctx), free_param, barcode(), cov);
    }

    /// @returns the global track parametrization from a bound representation
    DETRAY_HOST_DEVICE
    constexpr auto bound_to_free_param(
        const context &ctx, const bound_vector_type &bound_vec) const {
        return visit_mask<typename kernels::bound_to_free_param>(transform(ctx),
                                                                 bound_vec);
    }
    ...
    constexpr auto free_to_bound_jacobian(
        const context &ctx, const free_param_type &free_param) const {
        return this
            ->template visit_mask<typename kernels::free_to_bound_jacobian>(
                transform(ctx), free_param);
    }
