...
template <concepts::arithmetic depth = std::uint8_t,
          typename ratio_t = std::ratio<16, 9>>
class raw_image {

    public:
    using color = texture::color<depth>;
    ...
    DETRAY_HOST_DEVICE
    raw_image(const unsigned int height, const color c = {})
    ...
    /// Pixel data
    std::vector<color> m_data{m_height * m_width, color{}};
};
