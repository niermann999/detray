...
GTEST_TEST(detray_propagator, jacobian_cartesian2D) {
    ...
    ASSERT_NEAR(getter::element(bound_vec, 0u, 0u), 2.f, isclose);
    ASSERT_NEAR(getter::element(bound_vec, 1u, 0u), 4.f, isclose);
    ...
    ASSERT_NEAR(getter::element(bound_vec, 4u, 0u), -1. / 3.7416574f, isclose);
