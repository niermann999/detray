...
TEST(ALGEBRA_PLUGIN, toy_geometry) {
    ...
    auto& rectangles = detail::get<0>(masks);
    auto& trapezoids = detail::get<1>(masks);
    auto& annuli = detail::get<2>(masks);
    auto& cylinders = detail::get<3>(masks);
    auto& discs = detail::get<4>(masks);
    ...
    EXPECT_EQ(transforms.size(), 3244);
    ...
    darray<scalar, 6> bounds = {0., 27., -3000., 3000., -M_PI, M_PI};
    ...
    range = {8, 14};
    test_portal_links(vol_itr->index(), surfaces.begin() + range[0], range,
                      range[0], {3, 8}, {{14, inv_sf_finder}, {15, inv_sf_finder}, {16, inv_sf_finder}, {17, inv_sf_finder}, {18, inv_sf_finder}, {19, inv_sf_finder}});
